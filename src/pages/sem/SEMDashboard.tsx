...
function MetricCard({ label, value, sub }: { label: string; value: string; sub?: string }) {
  return (
    <div className="rounded-xl border border-stroke bg-white px-5 py-4 dark:border-strokedark dark:bg-boxdark">
...
export function SEMDashboard() {
  ...
  return (
    ...
    <div className="mx-auto max-w-screen-2xl p-6">
      ...
      <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          {state.selectedAccount?.name && (
            <p className="mt-0.5 text-sm font-medium text-body dark:text-bodydark">
              {state.selectedAccount.name}
            </p>
          )}
        </div>
      ...
      <div className="mb-6 rounded-xl border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
        <div className="border-b border-stroke px-6 py-5 dark:border-strokedark">
          <h3 className="font-semibold text-black dark:text-[#E2E5E9]">Google Ads</h3>
      ...
      <div className="mb-6 rounded-xl border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
        <div className="border-b border-stroke px-6 py-5 dark:border-strokedark flex items-center justify-between">
          <div>
            <h3 className="font-semibold text-black dark:text-[#E2E5E9]">Top Campaigns</h3>
    ...
    <div className="rounded-xl border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
        <div className="border-b border-stroke px-6 py-5 dark:border-strokedark flex items-center justify-between">
          <div>
            <h3 className="font-semibold text-black dark:text-[#E2E5E9]">Yearly Performance</h3>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex bg-gray-2 dark:bg-meta-4 rounded-lg p-1">
              <button
                ...
                className={`rounded-md px-4 py-1.5 text-xs font-semibold transition-colors ${activeTab === 'ads' ? 'bg-white text-black shadow-sm dark:bg-boxdark dark:text-[#E2E5E9]' : 'text-body dark:text-bodydark hover:text-black dark:hover:text-white'}`}
              >
                Google Ads
              </button>
              <button
                onClick={() => setActiveTab('guarantee')}
                className={`rounded-md px-4 py-1.5 text-xs font-semibold transition-colors ${activeTab === 'guarantee' ? 'bg-white text-black shadow-sm dark:bg-boxdark dark:text-[#E2E5E9]' : 'text-body dark:text-bodydark hover:text-black dark:hover:text-white'}`}
              >
                Google Guarantee
              </button>
            </div>
                  ...
                  if (row) {
                    return (
                      <tr key={monthName} className="hover:bg-gray-2 dark:hover:bg-meta-4 transition-colors">
                        <td className="whitespace-nowrap px-5 py-4 font-bold text-black dark:text-[#E2E5E9] uppercase text-xs bg-[#f8fafc] dark:bg-meta-4/80">{row.month}</td>
                        ...
                        <td className="whitespace-nowrap px-5 py-4 tabular-nums font-semibold text-black dark:text-[#E2E5E9]">{fmtCurrency(row.spend)}</td>
                    ...
                    return (
                      <tr key={monthName} className="hover:bg-gray-2 dark:hover:bg-meta-4 transition-colors">
                        <td className="whitespace-nowrap px-5 py-4 font-bold text-black dark:text-[#E2E5E9] uppercase text-xs bg-[#f8fafc] dark:bg-meta-4/80">{monthName}</td>
              ...
              {(() => {
                ...
                return (
                  <tr className="border-t-2 border-[#16a34a]/30 bg-[#eef7f2] dark:bg-[#1a382e]">
                    ...
                    <td className="whitespace-nowrap px-5 py-4 tabular-nums font-bold text-[#16a34a]">{n > 0 ? fmt(totalClicks) : dash}</td>
                    <td className="whitespace-nowrap px-5 py-4 tabular-nums font-bold text-[#16a34a]">{n > 0 ? fmt(totalConv, 2) : dash}</td>
                    <td className="whitespace-nowrap px-5 py-4 tabular-nums font-bold text-[#16a34a]">{n > 0 ? fmt(totalImpr) : dash}</td>
                    <td className="whitespace-nowrap px-5 py-4 tabular-nums font-bold text-[#16a34a]">{n > 0 ? `${fmt(weightedCtr, 2)}%` : dash}</td>
                    <td className="whitespace-nowrap px-5 py-4 tabular-nums font-bold text-[#16a34a]">{n > 0 ? fmtCurrency(weightedCpc) : dash}</td>
                    <td className="whitespace-nowrap px-5 py-4 tabular-nums font-bold text-[#16a34a]">{n > 0 ? fmt(totalInteract) : dash}</td>
                    <td className="whitespace-nowrap px-5 py-4 tabular-nums font-bold text-[#16a34a]">{avgOptScore > 0 ? `${fmt(avgOptScore, 2)}%` : dash}</td>
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="w-full max-w-lg rounded-2xl bg-white shadow-2xl dark:bg-boxdark overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="border-b border-stroke px-6 py-4 dark:border-strokedark flex items-center justify-between bg-gray-50 dark:bg-meta-4/20">
              <h4 className="font-bold text-black dark:text-[#E2E5E9] flex items-center gap-2">
            ...
            <div className="p-6">
              <textarea
                autoFocus
                className="w-full min-h-[180px] rounded-xl border border-stroke bg-gray-50 p-4 text-sm outline-none focus:border-[#1A72D9] focus:bg-white dark:border-strokedark dark:bg-boxdark-2 dark:text-[#E2E5E9] dark:focus:border-[#1A72D9] transition-all resize-none"
