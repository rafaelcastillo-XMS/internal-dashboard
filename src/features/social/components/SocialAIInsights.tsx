...
function cacheKey(accountId: string, platforms: string[]) {
  return `social-ai-insights:${accountId}:${[...platforms].sort().join(',')}`
}

export function SocialAIInsights({ accountName, accountId, platforms, metrics, posts }: Props) {
  const [loading, setLoading] = useState(false)
  const [insights, setInsights] = useState<Insights | null>(null)
  const [pinned, setPinned] = useState(false)
  const [error, setError] = useState('')

  const key = cacheKey(accountId, platforms)

  useEffect(() => {
    setInsights(null)
    setError('')
    setPinned(false)
    if (!accountId || platforms.length === 0) return
    try {
  ...
  const generate = async () => {
    ...
    setError('')
    try {
      const res = await fetch('/api/ai/social-insights', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ accountName, platforms, metrics, posts: posts.slice(0, 10) }),
      })
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-[var(--text-muted)] mt-0.5">
              {hasData
                ? `Generate AI-driven social media recommendations for ${accountName}`
                : 'Select at least one platform to generate insights'}
            </p>
          </div>
        </div>
        <button
    ...
    return (
      <div className="mb-6 rounded-xl border-[3px] border-violet-200 dark:border-violet-800/50 bg-violet-100/80 dark:bg-violet-900/25 px-6 py-5 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-[var(--text-muted)] mt-0.5">Claude is generating social media recommendations</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="mb-6">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          {pinned ? (
            <button onClick={handleUnpin} className="flex items-center gap-1.5 text-xs font-medium text-amber-600 dark:text-amber-400 hover:opacity-70 transition-opacity">
              <PinOff className="w-3.5 h-3.5" />
              Unpin
            </button>
          ) : (
            <button onClick={handlePin} className="flex items-center gap-1.5 text-xs font-medium text-[var(--text-muted)] hover:text-[var(--text-primary)] transition-colors">
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        {timeframes.map(({ key: tfKey, label, sub, icon: Icon, color, headerBg, border, iconBg, dot }) => {
          const items = insights?.[tfKey] ?? []
          return (
            <div key={tfKey} className={`rounded-xl border ${border} bg-[var(--bg-surface)] dark:bg-white/[0.02] overflow-hidden`}>
