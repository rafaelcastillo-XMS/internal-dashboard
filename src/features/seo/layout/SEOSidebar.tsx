...
import { NotepadText, ArrowLeft, ChevronDown } from 'lucide-react'
    ...
    items: [
      {
        ...
        href: '/seo/audit/initial-analysis',
        icon: (
...
const defaultOpenSections = NAV_GROUPS.reduce<Record<string, boolean>>((sections, group) => {
  if (group.section) sections[group.section] = true
  return sections
}, {})
...
export function SEOSidebar() {
  ...
  const reportsActive = location.pathname === '/seo/reports'
        ...
        <nav className={`flex-1 ${collapsed ? 'px-0 py-2' : 'px-3'} overflow-y-auto custom-scrollbar pb-2 space-y-0.5`}>
          ...
          {NAV_GROUPS.map((group, gi) => (
            <div key={gi}>
                  ...
                  group.collapsible ? (
                    <button
                      ...
                      onClick={() => setOpenSections((current) => ({
                        ...current,
                        [group.section as string]: !current[group.section as string],
                      }))}
                      className="flex w-full items-center gap-2 px-3 pt-4 pb-1.5 text-left"
                      aria-expanded={!!openSections[group.section]}
                    >
                      <h2 className="text-[10px] font-bold uppercase tracking-[0.18em] text-[var(--sidebar-section-label)]">{group.section}</h2>
                      <div className="flex-1 h-px bg-[var(--border)]" />
                      <ChevronDown
                        className={`h-3.5 w-3.5 shrink-0 text-[var(--sidebar-section-label)] transition-transform ${openSections[group.section] ? 'rotate-180' : ''}`}
                      />
                    </button>
                  ) : (
                    <div className="flex items-center gap-2 px-3 pt-4 pb-1.5">
              ...
              {(collapsed || !group.collapsible || !group.section || openSections[group.section]) && (
        ...
        <div className={`${collapsed ? 'px-0 py-3' : 'px-3 pb-4 pt-3'} border-t border-[var(--sidebar-border)] space-y-0.5`}>
          <div className="relative group">
            <Link
              to="/seo/reports"
              onClick={closeMobile}
              className={`${navItemClass} ${reportsActive ? activeClass : inactiveClass}`}
            >
